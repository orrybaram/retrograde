...
const ShipSpawn = preload("res://scripts/ShipSpawn.gd")
...
func _on_generation_complete() -> void:
	...
	if has_override:
		...
		# Try to find ShipSpawn locations first
		...
		if not spawn_locations.is_empty():
			...
			# Calculate spawn position from SolarSystemData (fallback)
			spawn_position = _calculate_spawn_position()
...
## Calculate spawn position from SolarSystemData or ShipSpawn locations
func _calculate_spawn_position() -> Vector2:
	# First priority: use ShipSpawn locations if available
	var spawn_locations = _find_ship_spawn_locations()
	if not spawn_locations.is_empty():
		var best_spawn = _select_best_spawn_location(spawn_locations)
		if best_spawn:
			var spawn_position = best_spawn.get_spawn_position()
			var parent = best_spawn.get_parent()
			var parent_name = parent.name if parent else "Unknown"
			print("Spawning at ShipSpawn location on ", parent_name, " at: ", spawn_position)
