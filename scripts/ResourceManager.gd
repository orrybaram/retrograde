...
var _connected_nodes: Array[ResourceNode] = []
...
func _connect_to_resource_nodes() -> void:
	...
	for node in resource_nodes:
		if node is ResourceNode:
			...
			if resource_node in _connected_nodes:
				continue
			
			# Connect signal
			if not resource_node.resource_harvested.is_connected(show_gain_indicator):
				resource_node.resource_harvested.connect(show_gain_indicator)
				_connected_nodes.append(resource_node)
