...
`playtests/freight_reload.play`, `freight_void.play`, `freight_track.play` and `freight_abandon.play` cover Freight never being lost (docs/adr/0012): pieces survive reloads in place (saved carrying, the ship reloads in flight with the load clamped), a load stays clamped through the Void's edge and turns up 1 km inside it when the dark takes the ship, while loose Freight stops just inside it (or where it is, let go of past it), letting go marks a piece on the Chart and tracks it (re-clamping tracks HOME), and a hull abandoned while carrying keeps the load, stops dead, is marked and survives respawn/reload until salvage frees the piece. Helpers: `pt.freight()`, `pt.freight_near(pos, [radius])`, `pt.chart_marks()`. Headless runs tick physics slower than frames: use `wait 0.1`, not `frames`, after an input that has to land, and between two `hold action`s. A sleeping derelict only notices the ship every 180 frames: warp ~1500 px from it, `wait 3.5`, then warp in and `stage_harvest` at once (waiting right beside it lets the harvest cone's enter event fire early, and staging then flakes).
