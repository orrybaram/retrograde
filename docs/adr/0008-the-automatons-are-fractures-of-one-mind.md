...
# The Automatons are fractures of one mind

Every voice the player meets is the Titan. UNIT-7, the Trader, the Broken One and the Merchant are not separate characters and not masks the Titan puts on to deceive — they are **disconnected fragments of one mind that do not know they are the same thing**.

The Titan was switched off Module by Module. What stayed powered are pieces running the last shape of themselves they had, in separate bodies, with no knowledge of each other. UNIT-7 does not know it is the Trader. It is not performing; it sincerely believes it is UNIT-7.
...
- **Nobody lies.** Every fragment is sincere. The premise holds unchanged.
- **The Broken One gets a mechanism.** It is the fragment nearest the inner system, where the bleed between pieces is worst — the one where the seams show. `docs/DESIGN.md` §5.3 already says it gets "worse and clearer at the same time" as Modules come online; that line now has a reason behind it.
- **Powering a Module reconnects the fragments.** The player is not only waking a machine, they are reassembling a mind that has been dissociated for centuries. At Influence 5, UNIT-7 does not sound like UNIT-7 any more — not because it was lying, but because it is no longer a separate thing. Combined with ADR 0009, the player made that friend and then unmade it.
