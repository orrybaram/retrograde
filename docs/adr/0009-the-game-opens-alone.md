...
- **The opening has no clock, and does not get one.** Under ADR 0010 nothing drains, so there is no pressure at all. A lost player is pointed, not hurried: the mast answers a Sweep from beyond the range anything else does, faint at the edge and firming up with approach (`docs/OPENING.md` §4). Its range is the first thing to put in front of a real playtester.
